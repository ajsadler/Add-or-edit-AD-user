...
$igdoorslabel.Location = New-Object System.Drawing.Point(400,10)
...
$deptnoselectedlabel.Font = "Arial, 9"
...
function EachDeptInNew # For each dept in the 'new' array, transform it to the AD group name
{
    ...
    $script:newDepartmentAD = $script:newDepartmentAD | Select -Unique
...
function EachDeptInDel # For each dept in the 'del' array, transform it to the AD group name
{
    ...
    $script:delDepartmentAD = $script:delDepartmentAD | Select -Unique
...
function GoButtonNew # Write to the log ; create new user ; set the user attributes ; add proxy addresses ; add to Access Group ; add to zP groups ; write the Retention Policy script
{
...
function GoButtonEdit # Write to the log ; set the new job title and department ; add/remove zP groups ; change Access Group if selection was changed
{
